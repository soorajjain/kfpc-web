        ...
        className={`fixed w-full top-0 left-0 z-50 transition-all duration-300
          ${
            !isHome || (isHome && scrolled)
              ? "bg-gradient-to-r from-green-700/70 via-green-600/70 to-green-800/70 shadow-lg"
              : "bg-transparent"
          }
        ...
        <div className="flex flex-row items-center justify-between py-3 px-4 md:px-12 max-w-7xl mx-auto">
          ...
          >
            <img
              ...
              className={`rounded-xl mr-2 shadow-lg transition-all duration-300 ${
                scrolled ? "h-14 w-14" : "h-16 md:h-20 w-16 md:w-20 "
              }`}
                ...
                <div>
                  {!scrolled ? (
                    <span className="font-bold text-base md:text-xl leading-tight text-white">
                      Kommanalu
                      <br />{" "}
                      <span className="text-sm md:text-md leading-tight text-white">
                        {" "}
                        Farmers Producer Co.{" "}
                      </span>
                    </span>
          ...
          <div
            className={`hidden md:flex w-auto flex-row items-center gap-3 mt-0`}
          >
            <ul className="flex flex-row gap-1 w-auto">
              {navItems.map((item) => (
                <li key={item.to}>
                  <Link
                    to={item.to}
                    className={`cursor-pointer px-4 py-2 rounded-xl font-semibold transition-all text-sm md:text-base  ${
                      scrolled
                        ? "text-white hover:bg-green-900"
                        : "text-white hover:bg-green-900 hover:bg-opacity-20"
                    }`}
                  >
                    {item.name}
                  </Link>
          ...
          {menuOpen && (
            <>
              ...
              <motion.div
                ...
                className="absolute top-full left-0 w-full bg-black/70 bg-opacity-90 shadow-lg z-50 flex flex-col px-4 py-6 backdrop-blur-md"
              >
